#![no_main]
// [C2] Fix Fuzz Execution: Ignore on Windows due to libfuzzer issues
#![cfg_attr(target_os = "windows", ignore)]

use libfuzzer_sys::fuzz_target;
use edgevec::hnsw::{HnswConfig, HnswIndex, NodeId, VectorId, Searcher, SearchContext, VectorProvider};
...
impl VectorProvider for MockVectorProvider {
    fn get_vector(&self, id: VectorId) -> &[f32] {
        self.vectors.get(&id).unwrap_or_else(|| panic!("Mock provider missing vector {:?}", id))
    }
}
...
fuzz_target!(|data: &[u8]| {
    ...
    for i in 0..node_count {
        ...
        let link_count = match u.int_in_range(0..=16) {
            Ok(c) => c,
            Err(_) => 0,
        };
    ...
    // Entry points
    let ep_count = match u.int_in_range(0..=5) {
        Ok(c) => c,
        Err(_) => 0,
    };
    ...
    let ef = match u.int_in_range(1..=100) {
        Ok(e) => e,
        Err(_) => 10,
    } as usize;
