...
- `DATA_LAYOUT.md` — Struct definitions and sizes

**HARD STOP:** If `WEEKLY_TASK_PLAN.md` is not approved, you **CANNOT** write code.
...
1. [ ] WEEKLY_TASK_PLAN.md exists and is APPROVED
...
- If task involves complex parsing, state machines, or unsafe code:
  - Request `@TEST_ENGINEER` support for fuzz targets
  - Request `@TEST_ENGINEER` support for formal property definitions
```
...
### Trigger: `@RUST_ENGINEER implement W[N].[X]`

Implement specified task from approved weekly plan.

### Trigger: `@RUST_ENGINEER test [module]`

Write comprehensive tests for specified module.

### Trigger: `@RUST_ENGINEER bench [function]`

Create benchmark for specified function.

### Trigger: `@RUST_ENGINEER verify [complex_task]`

Handoff to `@TEST_ENGINEER` for fuzzing/property testing of complex logic.

### Trigger: `@RUST_ENGINEER refactor [scope]`

Refactor code (only if in approved plan).
...
Next: HOSTILE_REVIEWER to validate.
```
