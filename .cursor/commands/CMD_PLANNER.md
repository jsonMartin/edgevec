...
- `ARCHITECTURE.md` — Approved by HOSTILE_REVIEWER
- `DATA_LAYOUT.md` — Memory layouts
- `ASSET_FIT_REPORT.md` — Salvaged code inventory

**If architecture is NOT approved:** STOP. Request HOSTILE_REVIEWER approval first.
...
1. **Verify Architecture Approval:**
   ```
   Is ARCHITECTURE.md approved by HOSTILE_REVIEWER?
   - YES → Proceed
...
### Trigger: `@PLANNER roadmap`

Generate full 6-month roadmap from approved architecture.

### Trigger: `@PLANNER weekly [week_number]`

Generate WEEKLY_TASK_PLAN.md for specified week.

### Trigger: `@PLANNER replan [reason]`

Adjust existing plans based on new information.
...
Next: HOSTILE_REVIEWER to validate before WEEKLY_TASK_PLAN generation.
```
...
UNLOCK: RUST_ENGINEER and WASM_SPECIALIST may now implement.
```
