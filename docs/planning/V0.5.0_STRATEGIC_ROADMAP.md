...
## v0.6.0 Preview: Edge Database Evolution

### Strategic Vision

v0.6.0 will transform EdgeVec from a "vector search library" to a true **Edge Vector Database**. These features were identified from external user feedback and will differentiate EdgeVec significantly:

### v0.6.0 Planned Features

| Feature | Priority | Description | Differentiator |
|:--------|:---------|:------------|:---------------|
| **Lazy Loading via HTTP Range Requests** | HIGH | Don't load entire index to RAM. Use HTTP range requests to fetch only needed graph portions during search. | **KILLER FEATURE** — Query hosted static files without full download |
| **Filterable HNSW** | HIGH | In-graph filtering (Qdrant-style). Filter evaluation during HNSW traversal, not before/after. | Best-in-class filtering performance |
| **Streaming Persistence** | MEDIUM | Memory-mapped file support for indices >1GB. Lazy page loading from disk. | Large dataset support |
| **Field Statistics** | MEDIUM | Pre-computed field cardinality and value distributions for selectivity estimation. | Faster auto-strategy selection |

### Lazy Loading Architecture Preview

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    LAZY LOADING VIA HTTP RANGE REQUESTS                     │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Current (v0.5.0):                                                          │
│  ─────────────────                                                          │
│  1. Download entire index file (100MB for 100k vectors)                     │
│  2. Parse into RAM                                                          │
│  3. Search                                                                  │
│  Problem: 100MB download before first search!                               │
│                                                                             │
│  Future (v0.6.0):                                                           │
│  ─────────────────                                                          │
│  1. Download index header only (~4KB)                                       │
│  2. On search: fetch needed graph pages via HTTP Range                      │
│  3. Cache frequently-accessed pages                                         │
│  4. LRU eviction for memory limits                                          │
│                                                                             │
│  Result: <1MB download for first search!                                    │
│                                                                             │
│  ┌──────────────────┐                                                       │
│  │  EdgeVec WASM    │                                                       │
│  │  ┌────────────┐  │     HTTP Range: bytes=0-4095                          │
│  │  │   Search   │──┼─────────────────────────────────►  ┌──────────────┐  │
│  │  │   Engine   │  │                                    │   CDN/S3     │  │
│  │  └────────────┘  │◄─────────────────────────────────  │  Static File │  │
│  │  ┌────────────┐  │     Range Response (4KB)           └──────────────┘  │
...
│  ├── Fixed-size page alignment (4KB pages)                                  │
│  ├── Page directory at file start                                           │
│  ├── Graph nodes grouped by locality                                        │
│  └── Metadata stored in separate pages (optional fetch)                     │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| Feature | EdgeVec v0.5.0 | EdgeVec v0.6.0 | Orama | LanceDB | Qdrant |
|:--------|:---------------|:---------------|:------|:--------|:-------|
| WASM Native | Yes | Yes | Yes | Partial | No |
| Metadata Filtering | Yes | Yes | Yes | Yes | Yes |
| In-Graph Filtering | No (hybrid) | Yes | No | No | Yes |
| Lazy Loading | No | **Yes** | No | No | No |
| Bundle Size | ~300KB | ~350KB | ~400KB | ~2MB | N/A |

**v0.6.0 positioning: "The only WASM vector database that queries hosted indices without full download."**

### v0.6.0 Timeline (Tentative)

| Week | Focus | Deliverables |
|:-----|:------|:-------------|
| Week 25-26 | Filterable HNSW | In-graph filtering implementation |
| Week 27-28 | Lazy Loading Design | File format v0.4, HTTP range architecture |
| Week 29-30 | Lazy Loading Implementation | Range fetcher, page cache, LRU eviction |
| Week 31-32 | v0.6.0 Release | Testing, documentation, publish |
...
| 1.1.0 | 2025-12-17 | Added v0.6.0 preview with lazy loading and Filterable HNSW |
